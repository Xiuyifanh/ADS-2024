...
class SpecialNumbers:
    ...
    def merger(start, end):  # merger function for the lists of prime numbers and palindromes to find prime palindromes
        ...
        for palindrome in palindromes:
            is_prime = True  # assumption statement that the palindrome is prime initially.
            ...
            if palindrome in {2, 3, 5, 7, 11}:
                ...
                continue
            for prime in primes:  # Loop through each prime number.
                if prime * prime > palindrome:  # If prime squared is greater than the palindrome, break the loop
                    break
                if palindrome % prime == 0:  # If the palindrome is divisible by any prime number, it is not prime
                    is_prime = False
                    break
            if is_prime:  # If the palindrome is prime, add it to the list of special numbers.
                prime_palindromes.append(palindrome)
        return prime_palindromes  # Return the list of special numbers.
